...
while (usedIds.has(candidate) && candidate <= BLOCK_BASE + maxCap) {
candidate++;
}
if (candidate > BLOCK_BASE + maxCap) {
return null;
}
return candidate;
}
...
if (!Number.isInteger(ruleId) || ruleId <= BLOCK_BASE) {
  return null;
}
...
if (src === dst || CorsairSecurity.sameOrSubdomain(dst, src)) {
  return { ok: false, error: 'cannot-block-self-or-subdomain' };
}
...
const existing = currentRules.find(r =>
  r.condition?.initiatorDomains?.includes(src) &&
  r.condition?.requestDomains?.includes(dst)
);
...
if (!assignedId && stagedReg.rules[pairKey]) {
  ...
  const chromeOwnerMatches = !chromeRuleWithId ||
    (chromeRuleWithId.condition?.initiatorDomains?.includes(src) &&
     chromeRuleWithId.condition?.requestDomains?.includes(dst));

  if ((!revOwner || revOwner === pairKey) && chromeOwnerMatches) {
    assignedId = stagedId;
  }
}
...
if (!assignedId) {
  ...
  if (currentRules.length >= maxCap) {
    await reconcileRegistryUnlocked();
  }
  assignedId = computeNextId(stagedReg.rules, stagedReg.reverse, await listCorsairRules(), maxCap);
  if (!assignedId) {
    return { ok: false, error: 'dnr-quota-saturated', saturated: true };
  }
}
...
try {
  ...
  if (registrySaved) {
    try {
      await _unlockedStorage.saveDnrRegistryUnlocked(originalReg);
    } catch {}
  }
...
if (ruleId) {
  try {
    await chrome.declarativeNetRequest.updateDynamicRules({ removeRuleIds: [Number(ruleId)] });
  } catch {}
...
if (toRemoveIds.length > 0) {
  try {
    await chrome.declarativeNetRequest.updateDynamicRules({ removeRuleIds: toRemoveIds });
  } catch {}
  await _unlockedStorage.saveDnrRegistryUnlocked(stagedReg);
}
...
try {
if (typeof _unlockedStorage.ensureReadyUnlocked === 'function') {
await _unlockedStorage.ensureReadyUnlocked();
}
profiles = _unlockedStorage.getProfilesUnlocked();
} catch (err) {
return {
ok: false,
error: 'storage-unhydrated: ' + err.message,
reconciled: false,
rulesPreserved: true
};
}
...
for (const removeId of toRemoveRuleIds) {
  ...
  if (pKey) {
    if (stagedReg.rules[pKey] === removeId) {
...
for (const [pKey, rId] of Object.entries(stagedReg.rules)) {
  if (!validPairs.has(pKey)) {
    delete stagedReg.rules[pKey];
    if (stagedReg.reverse[rId] === pKey) {
      delete stagedReg.reverse[rId];
    }
    if (stagedReg.meta) delete stagedReg.meta[pKey];
  }
}
...
for (const [pairKey, { source, destination }] of validPairs.entries()) {
  ...
  if (id && stagedReg.reverse[id] && stagedReg.reverse[id] !== pairKey) {
    id = null;
  }
  if (id && toRemoveRuleIds.has(Number(id))) {
    id = null;
  }
  ...
  if (!id) {
    ...
    if (activeRulesInPlay.length >= availableCap) {
      id = null;
    } else {
      id = computeNextId(stagedReg.rules, stagedReg.reverse, activeRulesInPlay, availableCap);
    }
...
if (toRemoveRuleIds.size > 0 || cleanToAddRules.length > 0) {
  try {
    ...
    return {
      ok: false,
      error: 'browser-update-failed: ' + browserErr.message,
      reconciled: false
    };
  }
}
...
return CorsairStorage.withTransactionGateExclusive(async () => {
  ...
  try {
    ...
    if (!reconRes.ok) {
      throw new Error('dnr-reconcile-failed: ' + reconRes.error);
    }
    return { ok: true, profile: mutated };
  } catch (err) {
