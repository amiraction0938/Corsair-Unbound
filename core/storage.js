const CorsairStorage = (() => {
  ...
  const KEYS = {
    ...
    userBlocked: 'userBlockedDomains',
  ...
  function recordLockEvent(event, name) {
    if (_lockObserver) {
      try { _lockObserver(event, name); } catch {}
    }
  }
  ...
  // Note: `_version` is the canonical settings schema version. The old
  // `version` field is intentionally absent — it was never incremented
  // by patchSettings() and its presence caused confusion.
  ...
  async function _deriveAesKey() {
    ...
    _aesKeyPromise = (async () => {
      const id = (typeof chrome !== 'undefined' && chrome.runtime?.id) || 'corsair-fallback-key';
  ...
  async function _encryptString(plain) {
    ...
    return { iv: _bufToB64(iv), data: _bufToB64(enc) };
  }
  ...
  function _xorObfuscate(str, keyStr) {
    let out = '';
  ...
  function _xorDeobfuscate(b64, keyStr) {
    try {
  ...
  try {
    await ensureReady();
      const legacy = (_settingsCache?.[LEGACY_SETTINGS_API_KEY] || '').trim();
      if (!legacy) return '';
      await setApiKeySecure(legacy);
      const next = { ..._settingsCache };
      delete next[LEGACY_SETTINGS_API_KEY];
      _settingsCache = next;
  ...
  async function patchSettings(patch) {
    ...
    return withTransactionGateShared(async () => {
      ...
      return withPartitionLock('globalSettings', async () => {
        ...
        if (LEGACY_SETTINGS_API_KEY in safePatch) delete safePatch[LEGACY_SETTINGS_API_KEY];
  ...
  /**
   * Overwrite the telemetry partitions (events / evidence / graph / chains)
   * with a caller-supplied payload. Used by the backup-restore flow when
   * the user has opted in to restoring telemetry. Must be called from
   * inside an exclusive transaction gate — this function does not acquire
   * the gate itself.
   */
  ...
  /**
   * Persist the configuration partitions that came out of a backup file.
   * This used to write ONLY profiles / settings / regression cases — which
   * meant any backup-restore silently dropped the user's custom domain
   * scripts, trusted domains, blocked domains, and threat cache. Now it
   * writes every configuration partition that the export side emits.
   */
  async function commitImportConfig(candidate) {
    ...
    const batch = {
      ...
      [KEYS.cases]: candidate.regressionCases
        ...
        // Reset session key on import — the backup may carry a legacy
    // plaintext vtApiKey inside settings, and we want the migration
    // path to pick it up on the next getApiKeySecure() call instead
    // of being blocked by a stale "already checked" flag.
    try { await chrome.storage.session.remove('vtApiKeySecure'); } catch {}

    if (candidate.customDomainScripts && typeof candidate.customDomainScripts === 'object') {
      batch.customDomainScripts = candidate.customDomainScripts;
    }
    if (Array.isArray(candidate.userTrustedDomains)) {
      batch.userTrustedDomains = candidate.userTrustedDomains;
    }
    if (Array.isArray(candidate.userBlockedDomains)) {
      batch.userBlockedDomains = candidate.userBlockedDomains;
    }
    if (candidate.threatCache && typeof candidate.threatCache === 'object') {
      batch.threatCache = candidate.threatCache;
    }
    // User preferences are stored outside globalSettings, so write
    // them here too. Only keys the backup explicitly carried are
    // written, so old v10 backups won't clobber the user's current
    // preferences.
    if (candidate.preferences && typeof candidate.preferences === 'object') {
      if (candidate.preferences.theme) batch.theme = candidate.preferences.theme;
      if (candidate.preferences.language) batch.language = candidate.preferences.language;
      if (candidate.preferences.tourCompleted === true) batch.tourCompleted = true;
      if (candidate.preferences.onboardingCompletedV1 === true) batch.onboardingCompletedV1 = true;
    }
    ...
    // If the imported settings contain a legacy plaintext VT key,
    // migrate it into the encrypted session store now — do not wait
    // for a lazy call from the next VT query.
    try {
      const legacy = (_settingsCache?.vtApiKey || '').trim();
      if (legacy) {
        await setApiKeySecure(legacy);
        const next = { ..._settingsCache };
        delete next.vtApiKey;
        _settingsCache = next;
        await safeSetWithQuotaRetry(KEYS.settings, _settingsCache);
      }
    } catch {}
  ...
  /**
   * Restore a snapshot that was captured before a failed import.
   * Symmetric with commitImportConfig — must restore every key that
   * commitImportConfig is capable of overwriting, otherwise a failed
   * import leaves the new (partial) data in place.
   */
  async function rollbackImportConfig(snapshot) {
    ...
    if (snapshot.customDomainScripts !== undefined) {
      batch.customDomainScripts = snapshot.customDomainScripts;
    }
    if (Array.isArray(snapshot.userTrustedDomains)) {
      batch.userTrustedDomains = snapshot.userTrustedDomains;
    }
    if (Array.isArray(snapshot.userBlockedDomains)) {
      batch.userBlockedDomains = snapshot.userBlockedDomains;
    }
    if (snapshot.threatCache && typeof snapshot.threatCache === 'object') {
      batch.threatCache = snapshot.threatCache;
    }
  ...
  return {
    ...
    getRegressionCases,
    saveRegressionCases,
    saveRegressionCasesUnlocked,
    commitImportConfig,
    rollbackImportConfig,
    replaceTelemetryUnlocked,
    getAgentContext,
