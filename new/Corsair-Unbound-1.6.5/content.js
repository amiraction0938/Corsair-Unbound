(function () {
  ...
  function hardRemoveBanner() {
    try {
      const el = document.getElementById(BANNER_HOST_ID);
      if (el) el.remove();
    } catch {}
  ...
  /* ============================================================
     LOADING BANNER (longest-lived — NOT auto-dismissed at 2s)
  ...
  function showLoadingBanner(host) {
    ...
    el.style.cssText = 'position:fixed;top:14px;right:14px;z-index:2147483647;pointer-events:auto;';
    ...
    shadow.innerHTML = `
      <style>
        ...
        .title { font-weight: 800; color: #60a5fa; font-size: 13px; letter-spacing: 0.02em; }
      ...
      <div class="card" role="status">
        <button class="close-loading" aria-label="Dismiss">✕</button>
        <div class="row">
          <div class="spinner"></div>
          <div class="body">
            <div class="title">Scanning Site…</div>
            <div class="host">${escapeHtml(host || _selfHost)}</div>
            <div class="progress-wrap"><div class="progress-fill"></div></div>
            <div class="status">Checking local cache…</div>
          </div>
        </div>
      </div>
    `;
    ...
    const closeBtn = shadow.querySelector('.close-loading');
    ...
    _progressTimer = setInterval(() => {
      ...
      if (status) {
        if (elapsed < 3000) status.textContent = 'Checking local cache…';
        else if (elapsed < 8000) status.textContent = 'Querying VirusTotal…';
        else if (elapsed < 15000) status.textContent = 'Waiting for rate-limit slot…';
        else if (elapsed < 25000) status.textContent = 'Analyzing 70+ security engines…';
        else status.textContent = 'Still working…';
      }
    }, 250);
  ...
  /* ============================================================
     VERDICT BANNER (auto-dismiss after 2s)
     ============================================================ */
  function showVerdictBanner(report) {
    ...
    const trustBtn = (isMalicious || report.verdict === 'suspicious')
      ? ''
      : `<button class="trust-btn" data-host="${escapeHtml(hostName)}">✓ Trust permanently</button>`;
    ...
    shadow.innerHTML = `
      <style>
        ...
        .title { font-weight: 800; color: ${style.color}; font-size: 13px; letter-spacing: 0.02em; }
        .host { font-family: 'SF Mono', Menlo, Consolas, monospace; font-size: 11px; color: #cbd5e1; margin-top: 3px; word-break: break-all; }
        ...
        .reasons div::before { content: '•'; color: ${style.color}; position: absolute; left: 0; }
        .actions {
      ...
      <div class="card" role="alert">
        <div class="row">
          <div class="icon">${escapeHtml(style.icon)}</div>
          <div class="body">
            <div class="title">${escapeHtml(style.label)}</div>
            <div class="host">${escapeHtml(hostName)}</div>
            <div class="risk">${escapeHtml(String(risk))}% risk</div>
            ${reasons.length ? `<div class="reasons">${reasons.map(r => `<div>${escapeHtml(r)}</div>`).join('')}</div>` : ''}
          </div>
        </div>
        <div class="actions">
          ${trustBtn}
    ...
    try {
      ...
      const tb = shadow.querySelector('.trust-btn');
      if (tb) {
        tb.addEventListener('click', async () => {
          const host = tb.getAttribute('data-host') || hostName;
          await sendBg({ type: 'trust-domain', host });
          try {
            tb.textContent = '✓ Trusted';
            tb.disabled = true;
