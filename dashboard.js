...
safeOn('runRegression','click',async()=>{const box=$('regression');if(!box)return;box.innerHTML='<div class="empty">Running regression cases...</div>';const r=await send({type:'run-regressions'});box.innerHTML='';if(!r||!r.ok){box.innerHTML='<div class="empty">Regression run failed.</div>';return;}const s=document.createElement('div');s.className='event';s.textContent='Passed '+(r.result?.passedCount||0)+' / '+(r.result?.total||0);box.appendChild(s);for(const item of (r.result?.results||[])){const row=document.createElement('div');row.className='event';const main=document.createElement('div');main.className='event-main';const t=document.createElement('div');t.className='event-type';t.textContent=item.title||item.caseId||'case';const m=document.createElement('div');m.className='event-meta';m.textContent=(item.passed?'PASS':'FAIL')+' • expected '+(item.expected?.verdict||'—')+' • actual '+(item.actual?.verdict||'—')+' • risk '+(item.actual?.risk==null?'—':item.actual.risk);main.append(t,m);row.appendChild(main);box.appendChild(row);}}
